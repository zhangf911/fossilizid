...
int main(){
	...
	Fossilizid::remoteq::ACCEPTOR acceptor = Fossilizid::remoteq::acceptor(que, ep);

	while (1){
		Fossilizid::remoteq::EVENT ev = Fossilizid::remoteq::queue(que);

		if (ev.type == Fossilizid::remoteq::event_type_accept){
			Fossilizid::remoteq::CHANNEL ch = Fossilizid::remoteq::accept(ev.handle.acp);
			
			Json::Value value;
			value["test"] = "ok";
			Fossilizid::remoteq::push(ch, value, Fossilizid::json_parser::json_to_buf);
		}
		else if (ev.type == Fossilizid::remoteq::event_type_recv){
			Json::Value ret;
			if (Fossilizid::remoteq::pop(ev.handle.ch, ret, Fossilizid::json_parser::buf_to_json)){
